...
static int cRandom()
{
    static QThreadStorage<QRandomGenerator*> generators;
    if (!generators.hasLocalData()) {
        generators.setLocalData(new QRandomGenerator(QRandomGenerator::securelySeeded()));
    }

    return generators.localData()->generate();
}
